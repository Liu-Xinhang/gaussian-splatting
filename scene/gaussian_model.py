...
class GaussianModel: ## 这个类存储的就是3d gaussian
    ...
    def create_from_bounding_box(self, bounding_box: np.ndarray, spatial_lr_scale: float, resolution: int):
        
    
        self.spatial_lr_scale = spatial_lr_scale
        point_cloud = sample_points_from_bounding_box(bounding_box, resolution)
        fused_point_cloud = torch.tensor(point_cloud).float().cuda()
        ...
        dist2 = torch.clamp_min(distCUDA2(torch.from_numpy(point_cloud).float().cuda()), 0.0000001)
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
        ...
        self.densify_and_clone(grads, max_grad, extent) ## 注意必须先clone再split
