...
def load_mesh(f, path_manager=None, device='cpu'):
    # if path_manager is None:
    #     path_manager = PathManager()
    mesh = load_objs_as_meshes([f], device=device)
...
class Renderer(nn.Module):
    def __init__(self,
        ...
        self.online_loading = online_loading
        if self.online_loading:
            self.load_meshed_path(mesh_dir, ".obj")
        else:
    ...
    def to(self, device):
        ...
        if not self.online_loading:
            for k in self.meshes:
                self.meshes[k] = self.meshes[k].to(device)
    
    def load_meshed_path(self, mesh_dirs, ext='.ply'):
        if not isinstance(mesh_dirs, list):
            mesh_dirs = [mesh_dirs]
        self.meshes = dict()
        for mesh_dir in mesh_dirs:
            mesh_dir = Path(mesh_dir)
            mesh_paths = mesh_dir.rglob(f"*{ext}")
            for mesh_path in mesh_paths:
                if 'MegaPose-GSO' in str(mesh_path):
                    obj_label = mesh_path.parts[-3]
                else:
                    obj_label = int(osp.basename(mesh_path).split('.')[0].split('_')[-1])-1
                self.meshes[obj_label] = mesh_path

    def load_meshes(self, mesh_dirs, ext='.ply'):
        if not isinstance(mesh_dirs, list):
            mesh_dirs = [mesh_dirs]
        self.meshes = dict()
        for mesh_dir in mesh_dirs:
            ...
            with tqdm.tqdm(total=len(mesh_paths), desc="loading model...") as pbar:
                for mesh_path in mesh_paths:
                    if 'MegaPose-GSO' in str(mesh_path):
                        ...
                        obj_label = int(osp.basename(mesh_path).split('.')[0].split('_')[-1])
    ...
    def forward(self, rotations, translations, internel_k, labels):
        ...
        if self.online_loading:
            meshes_to_render_list = [load_mesh(self.meshes[label], device=device) for label in labels.tolist()] #Todo 这一行代码如何并行
        else:
